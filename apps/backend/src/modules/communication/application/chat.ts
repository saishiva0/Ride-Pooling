...
import { createMessage, ensureRideChat, findChatAccess, findMessageForReport, findRideChat, markRead } from '../infrastructure/chat.repository.js';
...
export async function getRideChat(input: { rideId: string; userId: string }) {
  return prisma.$transaction(async (tx) => {
    ...
    return { rideId: result.ride.id, status: result.ride.status, closed: !canChatInRide(result.ride.status), conversation: result.conversation ? { id: result.conversation.id, messages: result.conversation.messages, lastReadAt: result.conversation.readStates[0]?.lastReadAt ?? null } : null };
  });
}

export async function sendRideChatMessage(input: { rideId: string; userId: string; text: string }) {
  if (typeof input.text !== 'string' || !isValidMessageText(input.text)) throw new ValidationError('text must contain 1–2000 non-whitespace characters', { field: 'text' });
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const participantIds = [ride.creatorId, ...ride.participants.map((p) => p.userId)].filter((id) => id !== input.userId);
    const blocked = (await Promise.all(participantIds.map((id) => findActiveBlockBetween(tx, input.userId, id)))).some(Boolean);
