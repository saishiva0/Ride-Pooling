...
import { CHAT_MESSAGE_MAX_LENGTH, canChatInRide, isValidMessageText, normalizeMessageText } from './chat-rules.js';
...
describe('chat rules', () => {
  ...
  it('normalizes and validates text limits', () => {
    ...
    expect(isValidMessageText('x'.repeat(CHAT_MESSAGE_MAX_LENGTH + 1))).toBe(false);
  });
});
