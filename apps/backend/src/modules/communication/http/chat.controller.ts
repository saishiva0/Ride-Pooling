...
export async function getRideChatHandler(
  req: Request,
  res: Response,
): Promise<void> {
...
export async function sendRideChatMessageHandler(
  req: Request,
  res: Response,
): Promise<void> {
  const identity = getAuthenticatedUser(res);
  ...
  sendData(
    res,
    201,
    await sendRideChatMessage({ rideId, userId: identity.userId, text }),
  );
}

export async function markRideChatReadHandler(
  req: Request,
  res: Response,
): Promise<void> {
  const identity = getAuthenticatedUser(res);
...
export async function reportRideChatMessageHandler(
  req: Request,
  res: Response,
): Promise<void> {
  const identity = getAuthenticatedUser(res);
