...
export function ChatScreen({ rideId }: ChatScreenProps) {
  ...
  const refresh = useCallback(async () => {
    try {
      setError(null);
      setChat(await api.getRideChat(rideId));
    } catch (err) {
  ...
  return (
    <Screen>
      <View style={styles.container}>
        ...
        {chat?.conversation === null && !chat.closed ? (
          <Text style={styles.muted}>
            Chat will be available once the ride has a confirmed participant.
          </Text>
        ) : null}
        {chat?.closed ? (
          <Text style={styles.muted}>
            This ride chat is closed. History remains available.
          </Text>
        ) : null}
        <ScrollView
        ...
        >
          {messages.map((message) => (
            <View
              ...
              style={[
                styles.bubble,
                message.senderId === userId ? styles.mine : null,
              ]}
            >
        ...
        {!chat?.closed && chat?.conversation ? (
          <View style={styles.composer}>
            <TextInput
              ...
              maxLength={2000}
              placeholder="Message ride participants"
              style={styles.input}
              editable={!sending}
            />
            <Pressable
